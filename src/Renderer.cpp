...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	std::vector<VkDescriptorSetLayoutBinding> computeLayoutbindings = { layoutBinding, culledLayoutBinding, numBladesLayoutBinding };
	...
	VkResult res = vkCreateDescriptorSetLayout(logicalDevice, &layoutInfo, nullptr, &computeDescriptorSetLayout);
...
void Renderer::CreateGrassDescriptorSets() {
	...
	grassDescriptorSetContainer.resize(scene->GetBlades().size());
	VkDescriptorSetLayout layouts[] = { modelDescriptorSetLayout };
	VkDescriptorSetAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = descriptorPool;
	allocInfo.descriptorSetCount = static_cast<uint32_t>(grassDescriptorSetContainer.size());
	allocInfo.pSetLayouts = layouts;

	// Allocate descriptor sets
	VkResult res = vkAllocateDescriptorSets(logicalDevice, &allocInfo, grassDescriptorSetContainer.data());
	assert(res == VK_SUCCESS);

	std::vector<VkWriteDescriptorSet> descriptorWrites(grassDescriptorSetContainer.size());
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i)
	{
		...
		descriptorWrites[i + 0].dstSet = grassDescriptorSetContainer[i];
...
void Renderer::CreateComputeDescriptorSets() {
	...
	computeDescriptorSetContainer.resize(scene->GetBlades().size());

	VkDescriptorSetLayout layouts[] = { computeDescriptorSetLayout };
	VkDescriptorSetAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = descriptorPool;
	allocInfo.descriptorSetCount = static_cast<uint32_t>(computeDescriptorSetContainer.size());
	allocInfo.pSetLayouts = layouts;

	VkResult res = vkAllocateDescriptorSets(logicalDevice, &allocInfo, computeDescriptorSetContainer.data());
	assert(res == VK_SUCCESS);

	std::vector<VkWriteDescriptorSet> descriptorContainer(3 * computeDescriptorSetContainer.size());

	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i)
	{
		VkDescriptorBufferInfo bladesBuffer = {};
		bladesBuffer.buffer = scene->GetBlades()[i]->GetBladesBuffer();
		bladesBuffer.offset = 0;
		bladesBuffer.range = NUM_BLADES * sizeof(Blade);

		VkDescriptorBufferInfo culledBladesBuffer = {};
		culledBladesBuffer.buffer = scene->GetBlades()[i]->GetCulledBladesBuffer();
		culledBladesBuffer.offset = 0;
		culledBladesBuffer.range = NUM_BLADES * sizeof(Blade);

		VkDescriptorBufferInfo numOfBlades = {};
		numOfBlades.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
		numOfBlades.offset = 0;
		numOfBlades.range = sizeof(BladeDrawIndirect);

		descriptorContainer[3 * i + 0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		descriptorContainer[3 * i + 0].dstSet = computeDescriptorSetContainer[i];
		descriptorContainer[3 * i + 0].dstBinding = 0;
		descriptorContainer[3 * i + 0].dstArrayElement = 0;
		descriptorContainer[3 * i + 0].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
		descriptorContainer[3 * i + 0].descriptorCount = 1;
		descriptorContainer[3 * i + 0].pBufferInfo = &bladesBuffer;
		descriptorContainer[3 * i + 0].pImageInfo = nullptr;
		descriptorContainer[3 * i + 0].pTexelBufferView = nullptr;

		descriptorContainer[3 * i + 1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		descriptorContainer[3 * i + 1].dstSet = computeDescriptorSetContainer[i];
		descriptorContainer[3 * i + 1].dstBinding = 1;
		descriptorContainer[3 * i + 1].dstArrayElement = 0;
		descriptorContainer[3 * i + 1].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
		descriptorContainer[3 * i + 1].descriptorCount = 1;
		descriptorContainer[3 * i + 1].pBufferInfo = &culledBladesBuffer;
		descriptorContainer[3 * i + 1].pImageInfo = nullptr;
		descriptorContainer[3 * i + 1].pTexelBufferView = nullptr;

		descriptorContainer[3 * i + 2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		descriptorContainer[3 * i + 2].dstSet = computeDescriptorSetContainer[i];
		descriptorContainer[3 * i + 2].dstBinding = 2;
		descriptorContainer[3 * i + 2].dstArrayElement = 0;
		descriptorContainer[3 * i + 2].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
		descriptorContainer[3 * i + 2].descriptorCount = 1;
		descriptorContainer[3 * i + 2].pBufferInfo = &numOfBlades;
		descriptorContainer[3 * i + 2].pImageInfo = nullptr;
		descriptorContainer[3 * i + 2].pTexelBufferView = nullptr;
	}
	vkUpdateDescriptorSets(logicalDevice, static_cast<uint32_t>(descriptorContainer.size()), descriptorContainer.data(), 0, nullptr);
}
...
void Renderer::CreateComputePipeline() {
	...
	descriptorSetLayouts.push_back(computeDescriptorSetLayout);
...
void Renderer::RecordComputeCommandBuffer() {
	...
	for (int i = 0; i < computeDescriptorSetContainer.size(); ++i)
	{
		vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 2, 1, computeDescriptorSetContainer.data(), 0, nullptr);
...
void Renderer::RecordCommandBuffers() {
	...
	for (size_t i = 0; i < commandBuffers.size(); i++) {
		...
		for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
			...
			vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelineLayout, 1, 1, &grassDescriptorSetContainer[j], 0, nullptr);
