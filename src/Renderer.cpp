...
void Renderer::CreateGrassDescriptorSetLayout() {
	...
	std::vector<VkDescriptorSetLayoutBinding> bindings = { bladeLayoutBinding };
...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	std::vector<VkDescriptorSetLayoutBinding> bindings = { bladeLayoutBinding, culledLayoutBinding, numBladesLayoutBinding };
...
void Renderer::RecordComputeCommandBuffer() {
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 1, 1, &computeDescriptorSets[i], 0, nullptr);
	}
