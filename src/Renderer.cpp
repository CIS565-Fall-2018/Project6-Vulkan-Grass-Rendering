...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	//The compute shader will perform culling: 
	//each thread read a blade, then determine whether it's culled or not, then write into a new array of blades
	//given this, where would be three layout descriptors:

	//the descriptor of input blades
	...
	//the descriptor of culled blades
	...
	//the descriptor of number of blades
	VkDescriptorSetLayoutBinding numAllBladesLayoutBinding = {};
	numAllBladesLayoutBinding.binding = 2;
	numAllBladesLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
	numAllBladesLayoutBinding.descriptorCount = 1;
	numAllBladesLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
	numAllBladesLayoutBinding.pImmutableSamplers = nullptr;

	//set of descriptors
	std::vector<VkDescriptorSetLayoutBinding>bindings = { inputBladesLayoutBinding, culledBladesLayoutBinding,numAllBladesLayoutBinding };

	//create layout
	VkDescriptorSetLayoutCreateInfo layoutInfo = {};
	layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
	//pNext should be nullptr, but the other descriptor creating functions didn't set it explicitly, is that ok?
...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
		...
		// the number of blades, as input to compute shader (compute)
...
void Renderer::CreateGrassDescriptorSets() {
	...
	//resize the grass descriptor vector
	grassDescriptorSets.resize(scene->GetBlades().size());
	
	// Describe the desciptor set
	//we don't have a function to create new descriptor set layout for grass
	//because grass is essentially also model, so we use model's descriptor set layout
	VkDescriptorSetLayout layouts[] = { modelDescriptorSetLayout };
	VkDescriptorSetAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = descriptorPool;
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		grassBufferInfo.buffer = scene->GetModels()[i]->GetModelBuffer();
...
void Renderer::CreateComputeDescriptorSets() {
	...
	if (vkAllocateDescriptorSets(logicalDevice, &allocInfo, computeDescriptorSets.data()) != VK_SUCCESS) {
		throw std::runtime_error("Failed = allocate descriptor set");
	}
	
	//configure for each descriptor
	std::vector<VkWriteDescriptorSet> descriptorWrites(3 * computeDescriptorSets.size());
	//for each blade, write into actuall blade buffers
	uint32_t bladesSize = scene->GetBlades().size();
	for (uint32_t i = 0; i < bladesSize; i++) {
		//input blades buffer
		...
		//visible (or culled) blades buffer
		...
		//num of all blades buffer
		VkDescriptorBufferInfo numAllBladesBufferInfo = {};
		numAllBladesBufferInfo.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
		numAllBladesBufferInfo.offset = 0;
		numAllBladesBufferInfo.range = sizeof(BladeDrawIndirect);
		...
		descriptorWrites[3 * i + 2].pBufferInfo = &numAllBladesBufferInfo;
