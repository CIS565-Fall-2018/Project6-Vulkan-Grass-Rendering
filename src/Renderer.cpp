...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	// Describe the binding of the input blades layout
...
void Renderer::RecordComputeCommandBuffer() {
	...
	for (size_t bladeId = 0; bladeId < scene->GetBlades().size(); bladeId++) {
		vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 2, 1, &computeDescriptorSets[bladeId], 0, nullptr);
		int warpId = ceil(NUM_BLADES / WORKGROUP_SIZE); // Ceil because group Ids start from 1
		vkCmdDispatch(computeCommandBuffer, warpId, 1, 1);
	}
