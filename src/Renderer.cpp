...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	if (vkCreateDescriptorSetLayout(logicalDevice, &info, nullptr, &computeDescriptorSetLayout) != VK_SUCCESS) {
...
void Renderer::CreateGrassDescriptorSets() {
	...
	alloInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	alloInfo.descriptorPool = descriptorPool;
	alloInfo.descriptorSetCount = static_cast<uint32_t>(grassDescriptorSets.size());
	alloInfo.pSetLayouts = layouts;

	if (vkAllocateDescriptorSets(logicalDevice, &alloInfo, grassDescriptorSets.data()) != VK_SUCCESS) {
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); i++) {
		...
		descriptorBufferInfo.buffer = scene->GetBlades()[i]->GetModelBuffer();
		descriptorBufferInfo.offset = 0;
		descriptorBufferInfo.range = sizeof(ModelBufferObject);
...
void Renderer::CreateComputeDescriptorSets() {
	...
	VkDescriptorSetAllocateInfo alloInfo = {};
	alloInfo.descriptorPool = descriptorPool;
	alloInfo.descriptorSetCount = computeDescriptorSets.size();
	alloInfo.pSetLayouts = layouts;
	alloInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;

	if (vkAllocateDescriptorSets(logicalDevice, &alloInfo, computeDescriptorSets.data()) != VK_SUCCESS) {
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); i++) {
		...
		VkDescriptorBufferInfo culledBladeBuffer = {};
		culledBladeBuffer.buffer = scene->GetBlades()[i]->GetCulledBladesBuffer();
		culledBladeBuffer.offset = 0;
		culledBladeBuffer.range = NUM_BLADES * sizeof(Blade);

		VkDescriptorBufferInfo numBladesBuffer = {};
		numBladesBuffer.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
		numBladesBuffer.offset = 0;
		numBladesBuffer.range = sizeof(BladeDrawIndirect);
		...
		descriptorWrites[3 * i].pBufferInfo = &bladesBuffer;
		...
		descriptorWrites[3 * i + 1].pBufferInfo = &culledBladeBuffer;
		...
		descriptorWrites[3 * i + 2].pBufferInfo = &numBladesBuffer;
